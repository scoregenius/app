...
const GameDetailScreen: React.FC = () => {
  ...
  /* A prediction is only meaningful before the game starts. Derived from the
   * clock rather than the feed: schedule rows often still carry no final score
   * hours after a game has ended, so keying off the score alone leaves a
   * finished game advertising a forecast for it. Matches game_card. */
  const hasStarted = useMemo(() => {
    const src = thisGame?.gameTimeUTC ?? thisGame?.game_date;
    if (!src) return false;
  ...
  /* Early states.
   * These sit below the hooks deliberately. They used to sit above, which
   * made useState and useMemo conditional: thisGame is undefined while the
   * schedule query is in flight and defined once it resolves, so the hook
   * count changed between renders and React would throw "rendered more
